...
PLANNER="$SCRIPTS/green_automation.py"
V1_RUNTIME="$SCRIPTS/green_automation_runtime.py"
MOVEIT_RUNTIME="$SCRIPTS/green_automation_moveit.py"
...
case "$MODE" in
    ...
    commission)
        echo "Commissioning green MoveIt trajectory cache."
        exec python3 -u \
            "$MOVEIT_RUNTIME" \
            --commission-cache
        ;;
    ...
    cached)
        echo "Running green automation with commissioned trajectories."
        exec python3 -u \
            "$MOVEIT_RUNTIME" \
            --use-cache
        ;;

...
from green_moveit_attached_box import CarriedBoxSceneManager
...
class GreenMoveItRuntime(Node):
    ...
    def __init__(
        ...
        super().__init__(
            'green_automation_moveit_runtime'
        )
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            'Execute the green sorting cycle through MoveIt.'
        )
    )
    ...
    mode_group.add_argument(
        ...
        help=(
            ...
            'green transfer and return trajectories.'
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        if arguments.commission_cache:
            GREEN_TRANSFER_CACHE.unlink(
                missing_ok=True
            )

            GREEN_RETURN_CACHE.unlink(
                missing_ok=True
            )
    ...
    try:
        ...
        if prepared_pickup is not None:
            ...
            node.get_logger().info(
                'Using PRECOMPUTED GREEN pickup IK: '
            ...
            node.get_logger().warning(
                'No valid precomputed GREEN pickup '
                'IK is available. Falling back to '
                'live perception and live IK.'
            )

            box_center = wait_for_green_box_pose(
                node
            )
        ...
        # pickup_exit -> green_bin_staging
        if arguments.commission_cache:
            node.get_logger().info(
                'Commissioning fixed attached-box '
                'green transfer trajectory.'
            )

            transfer_trajectory = node.execute_pose(
                'green_bin_staging',
                poses['green_bin_staging'],
            )
            ...
            save_trajectory(
                ...
                transfer_trajectory,
                label=(
                    'green attached transfer: '
                    'pickup_exit to green_bin_staging'
                ),
                metadata={
                    'start_pose': 'pickup_exit',
                    'goal_pose': 'green_bin_staging',
                    'carried_object': 'green_box_carried',
                },
            )

            node.get_logger().info(
                'Saved commissioned green transfer: '
                f'{GREEN_TRANSFER_CACHE}'
            )
        ...
        if green_slot.index == 0:
            ...
            node.execute_sequence(
                'PHASE 2: Dynamic green-bin placement',
                [
                    'drop_approach',
                    'drop_release',
                ],
        ...
        node.get_logger().info(
            'Retreating from the released green box.'
        )
        ...
        placed_object_id = (
            f'green_box_slot_{green_slot.index + 1:02d}'
        )
        ...
        # green_bin_staging -> pickup_exit
        if arguments.commission_cache:
            node.get_logger().info(
                'Commissioning fixed empty-tool '
                'green return trajectory.'
            )
            ...
            save_trajectory(
                ...
                label=(
                    'green empty return: '
                    'green_bin_staging to pickup_exit'
                ),
                metadata={
                    'start_pose': 'green_bin_staging',
            ...
            node.get_logger().info(
                'Saved commissioned green return: '
                f'{GREEN_RETURN_CACHE}'
            )
        ...
        node.get_logger().info(
            'GREEN MOVEIT AUTOMATION COMPLETE.'
        )
        ...
        node.get_logger().info(
            f'Green-bin slot used: '
            f'{green_slot.index + 1}/'
            f'{green_slot.capacity}'
        )
        ...
        node.get_logger().error(
            f'GREEN MOVEIT AUTOMATION FAILED: {error}'
        )
        return 1

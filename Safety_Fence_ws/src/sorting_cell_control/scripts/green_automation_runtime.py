...
def load_cached_plan() -> CachedPlan:
    if not PATH_FILE.is_file():
        raise RuntimeError(
            ...
            'Run 05_green_automation.sh plan first.'
        )
...
def green_box_half_height() -> float:
    ...
    for model in root.iter('model'):
        if model.attrib.get('name') != 'green_box':
            continue
    ...
    raise RuntimeError(
        'Could not read the green-box height from the world.'
    )


def wait_for_green_box_pose(
    ...
    node.get_logger().info(
        'Waiting for a settled GREEN box and live /perception/box_pose...'
    )
    ...
    if len(samples) < POSE_SAMPLE_COUNT:
        ...
        raise RuntimeError(
            'Timed out waiting for a settled green-box pose. '
    ...
    node.get_logger().info(
        'Averaged detected green-box centre: '
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            'Run green automation using the live detected box pose.'
        )
    )
    parser.add_argument(
        '--solve-only',
        action='store_true',
        help=(
            'Wait for the green box and validate live pickup IK '
    ...
    try:
        ...
        box_center = wait_for_green_box_pose(node)
        ...
        if arguments.solve_only:
            ...
            node.get_logger().info(
                'Run 05_green_automation.sh run to execute the pickup.'
            )
            return 0
        ...
        execute_phase(
            node,
            'PHASE 2: continuous transfer to green bin',
            [
                'pickup_approach',
                'middle',
                'drop_approach',
        ...
        node.get_logger().error(
            f'GREEN AUTOMATION FAILED: {error}'
        )
        return 1

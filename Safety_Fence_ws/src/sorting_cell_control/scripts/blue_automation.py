...
WAYPOINTS = CONFIG / 'blue_automation'
...
def blue_box_size(
    ...
    raise RuntimeError(
        'Cannot read blue-box dimensions.'
    )
...
def drop_geometry(
    ...
    blue_bottom = next(
        (
            ...
            if (
                ...
                == 'blue_bottom_collision'
            )
        ),
        None,
    )

    if blue_bottom is None:
        raise RuntimeError(
            'blue_bottom_collision not found.'
        )

    size_item = blue_bottom.find(
        './geometry/box/size'
    )
    ...
    blue_size = blue_box_size(
        world_root
    )

    # Place the box in the inner corner of the blue bin,
    ...
    if margin_x <= 0.0 or margin_y <= 0.0:
        raise RuntimeError(
            'The blue box does not fit inside the '
            'requested blue-bin corner clearance.'
        )


    # Use the same down-left bin corner as the
    # verified red and green drop paths.
    corner_offset_x = -margin_x
    corner_offset_y = margin_y
...
def generate_plan():
    ...
    print()
    print(
        'Solving IK: above blue drop zone'
    )
    ...
    print()
    print(
        'Solving IK: blue release at 1 mm'
    )
    ...
    plan = {
        ...
        'geometry': {
            ...
            'drop_location': (
                'blue_bin_inner_corner_closest_to_robot'
            ),
...
def execute(
    ...
    move_through(
        ...
        'Continuous motion to blue-box pickup',
    )
...
def print_plan(
    ...
    print()
    print(
        'BLUE AUTOMATION PATH GENERATED'
    )
    ...
    print(
        'Drop location: '
        'inner blue-bin corner closest to robot'
    )
...
def main():
    ...
    try:
        ...
        if arguments.mode == 'run':
            ...
            print(
                'BLUE AUTOMATION COMPLETED SUCCESSFULLY'
            )

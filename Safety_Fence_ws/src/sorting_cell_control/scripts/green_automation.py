...
WAYPOINTS = CONFIG / 'green_automation'
...
def green_box_size(
    ...
    raise RuntimeError(
        'Cannot read green-box dimensions.'
    )
...
def drop_geometry(
    ...
    green_bottom = next(
        (
            ...
            if (
                ...
                == 'green_bottom_collision'
            )
        ),
        None,
    )

    if green_bottom is None:
        raise RuntimeError(
            'green_bottom_collision not found.'
        )

    size_item = green_bottom.find(
        './geometry/box/size'
    )
    ...
    green_size = green_box_size(
        world_root
    )

    # Place the box in the inner corner of the green bin,
    ...
    if margin_x <= 0.0 or margin_y <= 0.0:
        raise RuntimeError(
            'The green box does not fit inside the '
            'requested green-bin corner clearance.'
        )

    # Select the bin corner physically closest to the robot.
    # This preserves the verified red-bin idea while allowing
    # the blue bin to use the opposite Y-side corner.
    bins_from_world = np.linalg.inv(world_from_bins)

    robot_local = (
        bins_from_world
        @ np.array([0.15, 0.0, surface_local_z, 1.0])
    )[:2]

    corner_offset_x = (
        -margin_x
        if robot_local[0] <= pose[0]
        else margin_x
    )

    corner_offset_y = (
        -margin_y
        if robot_local[1] <= pose[1]
        else margin_y
    )
...
def generate_plan():
    ...
    print()
    print(
        'Solving IK: above green drop zone'
    )
    ...
    print()
    print(
        'Solving IK: green release at 1 mm'
    )
    ...
    plan = {
        ...
        'geometry': {
            ...
            'drop_location': (
                'green_bin_inner_corner_closest_to_robot'
            ),
...
def execute(
    ...
    move_through(
        ...
        'Continuous motion to green-box pickup',
    )
...
def print_plan(
    ...
    print()
    print(
        'GREEN AUTOMATION PATH GENERATED'
    )
    ...
    print(
        'Drop location: '
        'inner green-bin corner closest to robot'
    )

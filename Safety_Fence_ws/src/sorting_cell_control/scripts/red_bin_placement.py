...
from red_automation import load_ik, solve_fixed


STATE_FILE = Path(
    '/tmp/safety_fence_red_bin_slots.json'
)

# Red-bin geometry from sorting_bins/model.sdf and
...
def generate_red_slots() -> List[RedBinSlot]:
...
def read_occupied_slots() -> Set[int]:
    ...
    try:
        ...
        raise RuntimeError(
            f'Cannot read red-bin slot state: {error}'
        ) from error
    ...
    if not isinstance(occupied, list):
        raise RuntimeError(
            'Red-bin state has no valid occupied list.'
        )

    slots = generate_red_slots()
    valid_indexes = {
        slot.index
        for slot in slots
    }
    ...
    for value in occupied:
        if (
            ...
            raise RuntimeError(
                f'Invalid red-bin slot index: {value!r}'
            )
...
def next_red_slot() -> RedBinSlot:
    occupied = read_occupied_slots()

    for slot in generate_red_slots():
...
def mark_red_slot_occupied(
    ...
    node.get_logger().info(
        f'Red-bin slot {slot.index + 1}/'
        f'{slot.capacity} marked occupied.'
    )


def restore_occupied_red_boxes(
    node,
    scene_manager,
    half_height: float,
) -> None:
    ...
    if not occupied:
        node.get_logger().info(
            'No occupied red-bin slots need restoration.'
        )
        return
    ...
    for index in sorted(occupied):
        ...
        object_id = (
            f'red_box_slot_{slot.index + 1:02d}'
        )
    ...
    node.get_logger().info(
        f'Restored {restored} occupied red-bin '
        'box collision object(s) into MoveIt.'
    )


def solve_dynamic_red_drop(
    node,
    cached,
    half_height: float,
) -> Tuple[

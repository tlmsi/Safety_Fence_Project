...
STATE_FILE = Path(
    '/tmp/safety_fence_blue_bin_slots.json'
)

# Blue-bin geometry from sorting_bins/model.sdf and
...
def generate_blue_slots() -> List[RedBinSlot]:
...
def read_occupied_slots() -> Set[int]:
    ...
    try:
        ...
        raise RuntimeError(
            f'Cannot read blue-bin slot state: {error}'
        ) from error
    ...
    if not isinstance(occupied, list):
        raise RuntimeError(
            'Blue-bin state has no valid occupied list.'
        )

    slots = generate_blue_slots()
    valid_indexes = {
        slot.index
        for slot in slots
    }
    ...
    for value in occupied:
        if (
            ...
            raise RuntimeError(
                f'Invalid blue-bin slot index: {value!r}'
            )
...
def next_blue_slot() -> RedBinSlot:
    occupied = read_occupied_slots()

    for slot in generate_blue_slots():
        if slot.index not in occupied:
            return slot

    raise RuntimeError(
        'The blue bin is full. '
        'No unused drop slots remain.'
    )


def mark_blue_slot_occupied(
    node,
    slot: RedBinSlot,
) -> None:
    occupied = read_occupied_slots()
    ...
    node.get_logger().info(
        f'Blue-bin slot {slot.index + 1}/'
        f'{slot.capacity} marked occupied.'
    )


def restore_occupied_blue_boxes(
    node,
    scene_manager,
    half_height: float,
) -> None:
    ...
    if not occupied:
        node.get_logger().info(
            'No occupied blue-bin slots need restoration.'
        )
        return
    ...
    for index in sorted(occupied):
        ...
        object_id = (
            f'blue_box_slot_{slot.index + 1:02d}'
        )
    ...
    node.get_logger().info(
        f'Restored {restored} occupied blue-bin '
        'box collision object(s) into MoveIt.'
    )


def solve_dynamic_blue_drop(
    node,
    cached,
    half_height: float,
) -> Tuple[

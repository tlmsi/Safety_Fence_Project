...
def red_box_half_height() -> float:
    ...
    raise RuntimeError(
        'Could not read the red-box height from the world.'
    )


def wait_for_red_box_pose(
    ...
    node.get_logger().info(
        'Waiting for a settled RED box and live /perception/box_pose...'
    )
    ...
    if len(samples) < POSE_SAMPLE_COUNT:
        ...
        raise RuntimeError(
            'Timed out waiting for a settled red-box pose. '
    ...
    node.get_logger().info(
        'Averaged detected red-box centre: '
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            'Run red automation using the live detected box pose.'
        )
    )
    parser.add_argument(
        '--solve-only',
        action='store_true',
        help=(
            'Wait for the red box and validate live pickup IK '
    ...
    try:
        ...
        box_center = wait_for_red_box_pose(node)
        ...
        if arguments.solve_only:
            ...
            node.get_logger().info(
                'Run 04_red_automation.sh run to execute the pickup.'
            )
            return 0
